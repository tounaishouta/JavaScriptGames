...
(function() {
  ...
  update = function() {
    var red, red2, _i, _j, _k, _len, _len1, _len2;
    frame++;
    for (_i = 0, _len = reds.length; _i < _len; _i++) {
      red = reds[_i];
      red.coulomb(blue);
      for (_j = 0, _len1 = reds.length; _j < _len1; _j++) {
        red2 = reds[_j];
        red.coulomb(red2);
      }
    }
    blue.update();
    for (_k = 0, _len2 = reds.length; _k < _len2; _k++) {
      red = reds[_k];
      red.update();
    }
    if (time != null) {
  ...
  Ball = (function() {
    ...
    function Ball(color, charge, x, y, wait) {
      this.color = color;
      this.charge = charge;
      this.x = x;
      this.y = y;
      this.wait = wait != null ? wait : 0;
      this.dx = 0;
      this.dy = 0;
    }
  ...
  addRanking = function() {
    ...
    if (rankOrder <= rankMax) {
      for (i = _i = rankMax; rankMax <= rankOrder ? _i < rankOrder : _i > rankOrder; i = rankMax <= rankOrder ? ++_i : --_i) {
  ...
  draw = function() {
    var color, i, red, scale, _i, _j, _len;
    ...
    switch (mode) {
      ...
      case "ranking":
        ...
        for (i = _j = 1; 1 <= rankMax ? _j <= rankMax : _j >= rankMax; i = 1 <= rankMax ? ++_j : --_j) {
          if (rankData[i] != null) {
  ...
  demo1 = function() {
    var back, cnt, controlArray, countMax, dtheta, f, generateArray, i, length, theta, _i, _j, _k, _l, _ref, _ref1;
    length = 100 * fps;
    generateArray = [];
    for (i = _i = 0, _ref = length / incRate; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
    ...
    dtheta = 1 / 4 * Math.PI;
    back = fps;
    for (cnt = _k = 0; 0 <= countMax ? _k < countMax : _k > countMax; cnt = 0 <= countMax ? ++_k : --_k) {
      f = evaluate(toFunction(generateArray), toFunction(controlArray));
      theta = controlArray[f].theta + dtheta;
      for (i = _l = _ref1 = f - back; _ref1 <= length ? _l < length : _l > length; i = _ref1 <= length ? ++_l : --_l) {
        controlArray[i] = {
          theta: theta,
          x: Math.cos(theta),
          y: Math.sin(theta)
  ...
  demo2 = function() {
    var control;
    control = function() {
      var dx, dy, r, r0, red, x, x0, y, y0, _i, _len;
      x0 = y0 = dx = dy = null;
      r0 = Infinity;
      for (_i = 0, _len = reds.length; _i < _len; _i++) {
        red = reds[_i];
